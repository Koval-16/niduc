...
def main():
    ...
    # Normalny przejazd dla 1 koła / szum = 1
    x = 0
    end = math.pi
    step = math.pi/90
    amp = 60
    ...
    # Normalny przejazd dla 1 koła / szum = 2
    x = 0
    end = math.pi
    step = math.pi / 90
    amp = 60
    ...
    while x <= end:
        ...
        if len(results_mn_1) == 0:
            results_mn_1.append(alg.M_out_of_N_algorithm(sensors, [1, 1, 1, 1, 1], 0.2, 2, 3, 0))
        else:
            results_mn_1.append(alg.M_out_of_N_algorithm(sensors, [1, 1, 1, 1, 1], 0.2, 2, 3, results_mn_1[-1]))
        if len(results_mn_2) == 0:
            results_mn_2.append(alg.M_out_of_N_algorithm(sensors, [1, 1, 1, 1, 1], 0.4, 4, 3, 0))
        else:
            results_mn_2.append(alg.M_out_of_N_algorithm(sensors, [1, 1, 1, 1, 1], 0.4, 4, 3, results_mn_2[-1]))

        if len(results_smoothing_2) == 0:
            results_smoothing_2.append(alg.smoothing_algorithm(sensors, 0, 3))
        else:
            results_smoothing_2.append(alg.smoothing_algorithm(sensors, results_smoothing_2[-1], 3))
    ...
    # Przejazd dla 1 koła, 1 z czujników zepsuty
    x = 0
    end = math.pi
    step = math.pi / 90
    amp = 60
    ...
    while x <= end:
        ...
        for i in range(4):
            sensors.append(err.random_error(y, noise))
        sensors.append(err.failure_error(y))
        print(f"{y:.2f} | Czujniki wskazuja: {[f'{value:.2f}' for value in sensors]}")
        results_majority_5.append(alg.majority_algorithm_tolerance(sensors, 0.5))
        results_majority_25.append(alg.majority_algorithm_tolerance(sensors, 0.25))
